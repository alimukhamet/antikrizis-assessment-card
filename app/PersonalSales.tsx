...
type Report = { name: string; periods: Period[]; noPlan?: boolean; generatedAt: string };
...
export default function PersonalSales({ mode }: { mode: 'results' | 'earnings' }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/personal-sales', { signal: controller.signal, cache: 'no-store' }).then(async response => {
  ...
  return <main className="personal-page"><div className="ps-wrap">
    ...
    <header className="ps-header"><div><h1>{earnings ? 'Мой заработок' : 'Мои результаты'}</h1><p>{report?.name || 'Ваши показатели'}</p></div><Link href={earnings ? '/my-results' : '/my-earnings'}>{earnings ? 'Результаты' : 'Заработок'} →</Link></header>
    <section aria-live="polite">
      {error ? <div className="ps-empty" role="alert"><p>{error}</p><button onClick={() => { setError(''); setRetry(retry + 1); }}>Повторить</button></div> : !report ? <p className="ps-empty" role="status">Загружаем вашу историю…</p> : report.noPlan ? <div className="ps-empty"><h2>Для вашего профиля план продаж не задан</h2><p>Вы вошли как {report.name}. Здесь показываются только личные результаты.</p></div> : <>
        {current && <article className="ps-current"><div className="ps-period"><h2>{range(current)}</h2><span>Текущий период</span></div>
          {earnings ? <><div className="ps-amount">{money(current.earned)}</div><p className="ps-sub">Начислено на сегодня · ставка {current.rate === null ? 'не рассчитана' : current.rate + '%'}</p>{details(current)}</> : <><div className="ps-result"><strong>{current.metric === 'count' ? number(current.count) : money(current.volume)}</strong><span>из {current.metric === 'count' ? number(current.target!) + ' договоров' : money(current.target)}</span><b>{current.progress === null ? '—' : Math.round(current.progress) + '%'}</b></div>{current.progress !== null && <progress max={100} value={Math.min(100,current.progress)} aria-label="Выполнение текущего плана" />}<p className="ps-sub">{current.remaining === 0 ? 'План выполнен' : current.remaining === null ? 'Не хватает данных для расчёта' : 'Осталось ' + (current.metric === 'count' ? number(current.remaining) + ' договоров' : money(current.remaining))}{current.metric === 'volume' ? ' · ' + current.count + ' договоров' : ''}</p></>}
        </article>}
