...
export default function PersonalSales({ mode }: { mode: 'results' | 'earnings' }) {
  ...
  function earningsDetails(p: EarningsMonth) { return <details className="ps-details"><summary>Подробнее</summary><div><p>{p.count} договоров на {money(p.volume)}</p><p>Оклад: {money(p.baseSalary)} · Комиссия: {money(p.commission)}</p><p>Бонус за 19 договоров: {money(p.contractBonus)}</p>{p.periods.map(period => <p key={period.id}>{range(period)}: ставка {period.rate === null ? 'не рассчитана' : period.rate + '%'} · комиссия {money(period.commission)}</p>)}{p.periods.some(period => period.leaderBonus) && <p>Отдельный бонус лидеру пока не включён.</p>}{p.missing > 0 && <p>Не указана сумма у {p.missing} договоров. Расчёт неполный.</p>}</div></details>; }
