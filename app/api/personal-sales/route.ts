...
export async function GET(request: Request) {
  ...
  if (!Object.hasOwn(PEOPLE, person)) return Response.json({ error: 'Выберите сотрудника.' }, { status: 400, headers });
  ...
  try {
    const plans = plansFor(person).filter(p => p.end.slice(0, 7) === month && p.start <= today);
    ...
    const uncovered = Math.round((Date.parse(range.end) - Date.parse(range.start)) / 86400000) + 1 - coveredDays.size;
