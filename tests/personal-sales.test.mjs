...
async function api(worker='ramazan',query='',fail=false,missing=0) {
 ...
 const compensation={compensationRepository:async()=>({payments:async()=>[],plans:async()=>[]}),storedPlan:row=>row};
 const route=await moduleAt('../app/api/personal-sales/route.ts',{require:name=>name.includes('sales-metrics')?metrics:name.includes('worker-session')?session:name.includes('sales-compensation')?compensation:{...rules,todayAlmaty:()=> '2026-09-10'}});
