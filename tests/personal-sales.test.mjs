...
test('personal route denies anonymous and cross-person requests without loading CRM',async()=>{for(const [worker,query,status]of [[null,'',401],['ramazan','person=darkhan',403],['ali','person=unknown',403]]){const {response,calls}=await api(worker,query);assert.equal(response.status,status);assert.equal(calls.length,0)}});
...
test('Ali never defaults to another manager; all personal history loads without a month picker',async()=>{const owner=await api('ali','');assert.equal(owner.data.name,'Ali');assert.equal(owner.data.noPlan,true);assert.deepEqual(owner.data.periods,[]);assert.equal(owner.calls.length,0);const all=await api('nurdaulet','');assert.equal(all.data.name,'Нурдаулет');assert.equal(all.data.canChoosePerson,false);assert.equal(all.data.periods.length,8);assert.equal(all.data.uncoveredDays,0);assert.ok(all.calls.every(u=>u.searchParams.get('managerId')==='4351'));});
