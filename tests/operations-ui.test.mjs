...
test('production portal starts with no client, no random deal, and no writes',async t=>{
 ...
 const controls=s.w.ServerDrafts.capture();assert.ok(controls.answers.length>=81);assert.equal(controls.groups.length,19); // + profilefamily (profile backfill only)
