...
test('native rules preserve contract evidence and withhold ambiguous total',()=>{const r=rules.extractNative(page(loan));assert.equal(r.credits.length,1);assert.equal(r.credits[0].contractNumber,'SYNTHETIC 1');assert.equal(r.credits[0].facts.find(f=>f.key==='monthlyPayment').value,'20.00');assert.equal(r.credits[0].facts.some(f=>f.key==='debtOutstanding'),false);assert.ok(!r.findings.includes('PAGE_COMPLETENESS_UNVERIFIED'));});
