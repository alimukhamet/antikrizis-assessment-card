import {test} from 'node:test';import assert from 'node:assert/strict';import fs from 'node:fs';import {JSDOM} from 'jsdom';import {compareGkb} from '../public/gkb-comparison.mjs';
test('comparison opens both source pages and navigates to the exact loan without changing answers',t=>{
 ...
 w.compareGkb=compareGkb;w.selectedFiles=[{id:1,person:'Клиент'},{id:2,person:'Клиент'}];w.HostedAssessment={getContext:()=>({client:{iin:'test',external:{dealId:'test-deal'}},assessmentDay:'2026-09-14'})};
