...
test('required document list retains EDS and conditional salary/benefit documents',async()=>{assert.equal(REQUIRED_DOCUMENTS.length,8);assert.ok(REQUIRED_DOCUMENTS.includes('ЭЦП файл'));const s=fixture();s.payload.docContext={social:'1',salary:'1'};const r=await s.run();assert.equal(r.required.length,10);assert.ok(r.missing.includes('Выписка зарплатного банка'));assert.equal(r.packageReady,false);assert.ok(r.issues.some(i=>i.code==='EDS_SEPARATE_UPLOAD_REQUIRED'));});
...
test('shortened IDs can satisfy the package only through one complete matching full report',async()=>{
 for(const fail of [false,true]){
  ...
  const r=await s.run();assert.equal(r.structurallyChecked.includes('ГКБ — краткий отчёт'),!fail);assert.equal(r.matchedShortReports.length,fail?0:1);assert.equal(r.packageReady,false); // Other documents/EDS still required.
