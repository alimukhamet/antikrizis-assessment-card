...
with sync_playwright() as p, TemporaryDirectory() as temp:
    ...
    page.evaluate(r'''source => {
        ...
        window.fetch=async(url,options)=>{
            ...
            if(options?.body&&JSON.parse(options.body).action==='generate')return{ok:true,json:async()=>({contract:{rendererVersion:ContractRenderer.version,data}})};
            return response;
        };
    }''', source)
    ...
    assert actions == ['generate','complete']
