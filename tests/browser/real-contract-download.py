...
with sync_playwright() as p, TemporaryDirectory() as temp:
    ...
    page.evaluate(r'''source => {
        ...
        window.fetch=async(url,options)=>{
            ...
            if(options?.body&&['complete','resume'].includes(JSON.parse(options.body).action))return{ok:true,json:async()=>({...await response.json(),contract:{rendererVersion:ContractRenderer.version,data}})};
            return response;
        };
    }''', source)
