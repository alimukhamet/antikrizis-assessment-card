...
SETUP = r'''() => {
 ...
 w.fetch=async(url,options)=>{
  ...
  if(b.action==='complete')row={...row,state:'verified',assessmentSaved:true,historySaved:true,contract:{rendererVersion:'a'.repeat(64),data:{client_name:'SAVED PERSON'}}};
...
def run():
    ...
    with sync_playwright() as p:
        ...
        assert state()['actions']==['prepare','complete']
        ...
        assert state()['actions'].count('complete')==2 and state()['actions'].count('prepare')==1
