...
function load(path,imports={}){const exports={};vm.runInNewContext(ts.transpileModule(fs.readFileSync(new URL('../'+path,import.meta.url),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022,esModuleInterop:true}}).outputText,{exports,require:n=>imports[n],crypto:webcrypto,TextEncoder,Uint8Array,Date,Map,Set});return exports;}
const documentRepo=load('lib/documents/repository.ts'),schema=JSON.parse(fs.readFileSync(new URL('../lib/questionnaire/schema.json',import.meta.url),'utf8'));const {validateDraft}=load('lib/questionnaire/draft.ts',{'./schema.json':schema,'../documents/repository':documentRepo});const {DraftRepository}=load('lib/questionnaire/repository.ts',{'../documents/repository':documentRepo});
