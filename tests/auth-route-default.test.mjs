...
test('hosted default delegates to payment-control without requiring a second local password',async()=>{
 ...
 '../../../lib/worker-session':{requestOriginAllowed:()=>true,issueSession:async()=> 'synthetic-signed-session',SESSION_COOKIE:'session',SESSION_SECONDS:60,LOCAL_WORKER_PASSWORDS:{azhar:'AZHAR_PASSWORD'}},
 ...
 vm.runInNewContext(ts.transpileModule(source,{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports,require:n=>deps[n],process:{env:{SITE_SESSION_TOKEN:'s'.repeat(32),AZHAR_PASSWORD:'synthetic-azhar'}},Response,TextDecoder,Uint8Array,JSON});
 ...
 assert.equal(response.status,200);assert.equal(configuration.provider,'payment-control');assert.equal(configuration.localPassword,undefined);assert.deepEqual({...configuration.workerPasswords},{azhar:'synthetic-azhar'});assert.match(response.headers.get('set-cookie'),/HttpOnly; Secure; SameSite=Strict/);
});
