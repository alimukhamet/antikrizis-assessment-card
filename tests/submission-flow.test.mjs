...
function setup({uncertain=false,editDuringPrepare=false}={}){
 ...
 w.HostedAssessment={ready:()=>true,getContext:()=>({client:{title:'CRM TITLE - [whatcrm] line #21',external:{dealId:'11665'}}})};w.ServerDrafts={capture:()=>payload,reviewBindings:()=>[]};w.URL.createObjectURL=()=> 'blob:test';w.URL.revokeObjectURL=()=>{};w.HTMLAnchorElement.prototype.click=()=>{downloads++;};
 ...
 w.fetch=async(url,options)=>{if(!options?.method)return{ok:true,json:async()=>({submission:row})};const b=JSON.parse(options.body);calls.push(b);if(b.action==='prepare'){row={requestId:b.requestId,state:'prepared',assessmentSaved:false,historySaved:false,contractNumber:'TEST',reviewText:'SAVED SNAPSHOT'};if(editDuringPrepare)payload={answers:['EDITED']};}if(b.action==='complete'){const pending=uncertain&&row.state==='prepared';row={...row,state:pending?'uncertain':'verified',assessmentSaved:!pending,historySaved:!pending,contract:pending?null:{rendererVersion:'a'.repeat(64),data:{client_name:'SAVED PERSON',contract_number:'TEST'}}};}if(b.action==='cancel')row={...row,state:'cancelled'};return{ok:true,json:async()=>row};};
