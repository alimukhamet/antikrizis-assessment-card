...
function setup({uncertain=false,editDuringPrepare=false}={}){
 ...
 w.fetch=async(url,options)=>{
  ...
  if(b.action==='complete'||b.action==='resume'){
   row={requestId:row?.requestId||b.requestId,state:'verified',assessmentSaved:true,historySaved:true,contractNumber:'TEST',reviewText:'SAVED SNAPSHOT',workflow:{status:'ready'},contract:{rendererVersion:'a'.repeat(64),data:{client_name:'SAVED PERSON'}}};
   if(uncertain&&b.action==='complete')row={...row,state:'uncertain',assessmentSaved:false,historySaved:false,workflow:{status:'retry',message:'Bitrix ещё не подтвердил сохранение'},contract:null};
   if(editDuringPrepare)payload={answers:['EDITED']};
  }else if(b.action==='cancel')row={...row,state:'cancelled'};
  else assert.fail('The browser must not orchestrate individual save steps: '+b.action);
...
test('one action saves fields, history and downloads the immutable contract snapshot',async()=>{const s=setup();s.check(false);assert.equal(s.save.disabled,false);s.check(true);await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['complete']);assert.equal(new Set(s.calls.map(c=>c.requestId)).size,1);assert.equal(s.rendered().data.client_name,'SAVED PERSON');assert.equal(s.downloads(),1);});
test('interrupted save resumes with readback, without another commit',async()=>{const s=setup({uncertain:true});s.check(true);await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['complete']);assert.equal(s.downloads(),0);await new Promise(setImmediate);await s.resume().onclick();assert.deepEqual(s.calls.map(c=>c.action),['complete','resume']);assert.equal(s.downloads(),1);});
test('editing after submission starts prevents stale download without trying to cancel a possibly sent write',async()=>{const s=setup({editDuringPrepare:true});s.check(true);await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['complete']);assert.equal(s.downloads(),0);});
test('download starts validation and saves draft documents before preparing the contract',async()=>{
 ...
 await s.save.onclick();assert.deepEqual(steps,['check','draft','documents','complete']);assert.equal(s.downloads(),1);
});
...
test('every outbound submission action includes the confirmed destination',async()=>{
 const s=setup();s.check(true);await s.save.onclick();assert.ok(s.calls.length>0);for(const call of s.calls)assert.deepEqual(call.destination,{dealId:'11665',iin:'SYNTHETIC',identityRevision:1});
});
...
test('real upload and submission flows recover a delayed document receipt and then download once',async()=>{
 ...
 await s.save.onclick();assert.equal(uploads.length,2);assert.equal(uploads[1].action,'reconcile');assert.equal(uploads[0].requestId,uploads[1].requestId);assert.deepEqual(s.calls.map(c=>c.action),['complete']);assert.equal(s.downloads(),1);s.w.close();
});
