...
function setup({uncertain=false,editDuringPrepare=false}={}){
 ...
 w.fetch=async(url,options)=>{
  ...
  if(b.action==='generate'){
   if(editDuringPrepare)payload={answers:['EDITED']};
   return{ok:true,json:async()=>({contract:{rendererVersion:'a'.repeat(64),data:{client_name:'SAVED PERSON'}}})};
  }
  if(b.action==='complete'){
   const pending=uncertain&&!row;
   row={requestId:b.requestId,state:pending?'uncertain':'verified',assessmentSaved:!pending,historySaved:!pending,contractNumber:'TEST',reviewText:'SAVED SNAPSHOT'};
  }
  if(b.action==='cancel')row={...row,state:'cancelled'};
...
test('one button downloads validated current answers then asks the server to synchronize',async()=>{const s=setup();s.check(false);assert.equal(s.save.disabled,false);s.check(true);await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['generate','complete']);assert.equal(new Set(s.calls.map(c=>c.requestId)).size,1);assert.equal(s.rendered().data.client_name,'SAVED PERSON');assert.equal(s.downloads(),1);});
test('an uncertain CRM save does not block the file, and saved-version recovery remains available',async()=>{
 const s=setup({uncertain:true});s.check(true);await s.save.onclick();
 assert.deepEqual(s.calls.map(c=>c.action),['generate','complete']);assert.equal(s.downloads(),1);
 assert.match(s.w.document.getElementById('status').textContent,/не подтверждено/);
 await new Promise(setImmediate);await s.resume().onclick();
 assert.deepEqual(s.calls.map(c=>c.action),['generate','complete','complete','contract']);assert.equal(s.downloads(),2);
});
test('editing answers during preparation cancels before any CRM write',async()=>{const s=setup({editDuringPrepare:true});s.check(true);await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['generate']);assert.equal(s.downloads(),0);});
test('download starts validation and saves draft documents before preparing the contract',async()=>{
 ...
 await s.save.onclick();assert.deepEqual(steps,['check','draft','generate','documents','complete']);assert.equal(s.downloads(),1);
});
test('incomplete answers block generation; failed CRM upload preserves the validated file and reports failure',async()=>{
 ...
 const failed=setup();failed.check(true);failed.w.AssessmentDocumentUpload={submit:async()=>{throw Error('document receipt missing');}};await failed.save.onclick();assert.deepEqual(failed.calls.map(c=>c.action),['generate']);assert.equal(failed.downloads(),1);assert.match(failed.w.document.getElementById('status').textContent,/Bitrix НЕ подтверждено/);
});
...
test('every outbound submission action includes the confirmed destination',async()=>{
 const s=setup();s.check(true);await s.save.onclick();for(const call of s.calls.filter(c=>['generate','complete'].includes(c.action)))assert.deepEqual(call.destination,{dealId:'11665',iin:'SYNTHETIC',identityRevision:1});
});
...
test('download exposes immediate busy/progress and retains a direct file link without repeating writes',async()=>{
 ...
 const pending=s.save.onclick();await new Promise(setImmediate);assert.equal(s.save.disabled,true);assert.equal(events.at(-1).busy,true);assert.equal(events.at(-1).message,'Договор готов. SAVING DOCUMENTS');assert.equal(s.downloads(),1,'File is available while CRM upload is pending');
 await s.save.onclick();assert.deepEqual(s.calls.map(c=>c.action),['generate']);finish(true);await pending;
...
test('real upload and submission flows recover a delayed document receipt and then download once',async()=>{
 ...
 await s.save.onclick();assert.equal(uploads.length,2);assert.equal(uploads[1].action,'reconcile');assert.equal(uploads[0].requestId,uploads[1].requestId);assert.deepEqual(s.calls.map(c=>c.action),['generate','complete']);assert.equal(s.downloads(),1);s.w.close();
});
...
test('an answer edited while the saved contract is fetched prevents rendering',async()=>{
 ...
 s.w.fetch=async(url,options)=>{const result=await fetch(url,options);if(options?.method&&JSON.parse(options.body).action==='generate')s.w.ServerDrafts.capture=()=>({answers:['CHANGED']});return result;};
 await s.save.onclick();assert.equal(s.downloads(),0);assert.equal(s.rendered(),null);s.w.close();
});
test('editing after download invalidates the direct link and never reports the changed version as synced',async()=>{
 const s=setup();s.check(true);const fetch=s.w.fetch;
 s.w.fetch=async(url,options)=>{const result=await fetch(url,options);if(options?.method&&JSON.parse(options.body).action==='complete'){s.w.ServerDrafts.capture=()=>({answers:['CHANGED']});s.flow.invalidate();}return result;};
 await s.save.onclick();assert.equal(s.downloads(),1);assert.equal(s.w.document.getElementById('downloadContractFile').hidden,true);
 assert.doesNotMatch(s.w.document.getElementById('status').textContent,/Карточка и история сохранены/);s.w.close();
});
...
test('a malformed generation response cannot fall back to an older saved contract',async()=>{
 const s=setup();s.check(true);const fetch=s.w.fetch;
 s.w.fetch=(url,options)=>options?.body&&JSON.parse(options.body).action==='generate'?Promise.resolve({ok:true,json:async()=>({})}):fetch(url,options);
 await s.save.onclick();assert.equal(s.downloads(),0);assert.equal(s.calls.length,0);
 assert.match(s.w.document.getElementById('status').textContent,/не вернул данные договора/);s.w.close();
});
