...
function setup({uncertain=false,editDuringPrepare=false}={}){
 ...
 return{w,flow,check,calls,save:[...w.document.querySelectorAll('button')].find(b=>b.textContent==='Сохранить и скачать'),resume:()=>[...w.document.querySelectorAll('button')].find(b=>b.textContent.startsWith('Продолжить сохранение')),rendered:()=>rendered,downloads:()=>downloads};
}
