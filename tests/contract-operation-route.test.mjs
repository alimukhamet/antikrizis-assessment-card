...
function load(file,imports){
 ...
 vm.runInNewContext(result.outputText,{exports,require:n=>{if(!(n in imports))throw Error('Unexpected import '+n);return imports[n];},process:{env:{}},Response,AbortSignal,fetch,setTimeout,clearTimeout});return exports;
}
function fixture(options={}){
 ...
 let contexts=0,operations=0;const legacy=[];let signals=[];
 ...
 const imports={
  ...
  '../../../../../lib/crm/assessment-intake-sync':{syncAssessmentIntake:async()=>assert.fail('No platform sync configured for synthetic test')},
  'cloudflare:workers':{env:{DB:{}}},
 };
 const route=load('app/api/assessment/[dealId]/submission/route.ts',imports);
 ...
 return{run,counts:()=>({contexts,operations}),legacy,signals:()=>signals.filter(Boolean)};
}
