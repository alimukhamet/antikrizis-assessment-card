...
for(const mode of ['contract','handoff'])test(mode+' draft restore preserves answers and every document reference',async()=>{
 ...
 for(const owner of ['client','partner']){
  const choice=d.querySelector(`[data-owner="${owner}"][data-holding="land"]`);choice.checked=true;choice.dispatchEvent(new w.Event('change',{bubbles:true}));
  const g=d.getElementById(owner+'land');g.querySelector(`[id^=${owner}LandDescription]`).value=owner+' TEST LAND';g.querySelector(`[id^=${owner}LandOwnership]`).value='share';g.querySelector(`[id^=${owner}LandShare]`).value='25';g.querySelector(`[id^=${owner}LandValue]`).value='0';g.querySelector(`[id^=${owner}LandPledged]`).value='Нет';
 }
 d.getElementById('clientland').querySelector('.add-row').click();d.querySelector('#clientland .repeat-rows').lastElementChild.querySelector('[id^=clientLandDescription]').value='SECOND TEST LAND';
 const other=d.querySelector('[data-owner="client"][data-holding="other"]');other.checked=true;other.dispatchEvent(new w.Event('change',{bubbles:true}));d.getElementById('n8017').value='SYNTHETIC TRAILER\nSYNTHETIC EQUIPMENT';
