...
import {httpHeaders} from './bitrix-headers-helper.mjs'; const exports={};vm.runInNewContext(ts.transpileModule(fs.readFileSync(new URL('../lib/crm/document-upload.ts',import.meta.url),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports,require:n=>n==='./http-headers'?httpHeaders:({sha256:async bytes=>createHash('sha256').update(bytes).digest('hex')}),Map,Set,Uint8Array,AbortSignal,btoa,TextEncoder,ReadableStream});const{readFileField,createDocumentUploadAdapter}=exports;
