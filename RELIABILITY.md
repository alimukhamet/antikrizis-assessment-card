...
| --- | --- | --- | --- |
| P1 | Scheduled monitoring cadence unverified | At 08:32 UTC on 23 September, no `schedule` event had run and the newest snapshot was over 45 minutes old. The workflow is active on `main`, Actions is enabled, and manual fallback `35837724070` passed with no incidents, stuck operations or missing active loans in the sampled saved case. At 08:33 UTC, default-branch commit `c0b010c` republished the same 15-minute cadence using equivalent `7/15` syntax; checkout, permissions and probe steps are unchanged. Default-branch check `35837835102` passed at 08:40 UTC; no automatic run was visible at 08:41 UTC. This is a bounded scheduler-registration repair attempt, not an established root cause or verified fix. | Observe a successful automatic scheduled run. Do not treat manual fallback success as proof of the 15-minute cadence. Track first automatic execution and later spacing separately from snapshot freshness. No service outage is established. |
