...
- Court-trend navigation, 23 September: the owner reported duplicate search/selection controls and requested ascending lists. The dropdowns repeated the sidebar, while courts used descending decision count and regions used alphabetical order. One search now feeds one region/court list on desktop and mobile, ordered by the latest displayed percentage ascending, with missing values last and zero preserved. Local browser checks reproduced region → district court and cross-region search, including 390-pixel layout without overflow. The navigation regression verifies ordering/ties/missing data without mutating source rows. The authenticated release audit requires one search, one navigation surface, the ascending-order label, no duplicate dropdowns and exact source-data parity. Production release evidence is in that commit's deploy workflow audit; local interaction checks do not claim an authenticated browser click-through.
