...
export async function searchClients(webhook:string,query:string,send:typeof fetch=fetch):Promise<RecentClient[]>{
 ...
 const result=await call(webhook,'crm.deal.list',{order:{DATE_MODIFY:'DESC'},filter,select:['ID','TITLE','DATE_MODIFY','UF_CRM_ANK_PRIMARY_DOCS',...contractFields]},send);
 ...
 return result.slice(0,50).flatMap(item=>{
  ...
  return [{dealId:String(item.ID),title:String(item.TITLE||''),updatedAt:String(item.DATE_MODIFY||''),fileCount:count,hasContract:hasPreparedContract(item)}];
 });
}
export async function draftContractStates(webhook:string,dealIds:string[],send:typeof fetch=fetch){
 const ids=[...new Set(dealIds)].filter(id=>/^[1-9]\d*$/.test(id)).slice(0,30);if(!ids.length)return new Map<string,boolean>();
 const result=await call(webhook,'crm.deal.list',{filter:{ID:ids},select:['ID',...contractFields]},send);
 if(!Array.isArray(result))throw new DocumentUploadError('DEAL_READ_FAILED');
 return new Map<string,boolean>(result.filter(item=>ids.includes(String(item.ID))).map(item=>[String(item.ID),hasPreparedContract(item)]));
}
