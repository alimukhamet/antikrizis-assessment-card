...
export async function analysisResponse(client:ClientContext,record:CaseRow,repository:EvidenceRepository,stored:{document:DocumentRow;extraction:ExtractionRow;result:unknown},cacheHit:boolean){
 ...
 const reviewContext={iin:extraction.identity.iin||client.iin||'',pages:read.totalPages,issuedAt:powerValidation?.issuedAt||extraction.issuedAt||'',expiresAt:powerValidation?.expiresAt||'',from:extraction.coverage?.from||extraction.bankStatement?.from||'',to:extraction.coverage?.to||extraction.bankStatement?.to||'',representative:extraction.power?.representative||null};
