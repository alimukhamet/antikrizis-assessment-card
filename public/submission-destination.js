window.SubmissionDestination={async confirm(options={}){
 ...
 const selected=await new Promise(resolve=>{
  ...
  const title=document.createElement('h2');title.id='submissionDestinationTitle';title.textContent=options.title||'Проверьте получателя';
  ...
  const note=document.createElement('p');note.textContent=options.note||'Ответы, документы и условия договора будут сохранены в этой сделке и её истории.';
  ...
  const cancel=document.createElement('button'),send=document.createElement('button');cancel.type=send.type='button';cancel.className='btn btn-ghost';send.className='btn btn-main';cancel.textContent='Назад';send.textContent=options.action||'Сохранить и скачать договор';send.disabled=true;
