...
function afRow(group,key,loan=null){
 ...
 if(candidates.length){
  ...
  for(const other of candidates.slice(1)){
   ...
   const entirelySourced=loan&&controls.every(e=>{const field=e.id.replace(/_r\d+$/,'');return e.type==='checkbox'?!e.checked:!e.value||String(loan.fields[field])===e.value||(field==='n8038'&&loan.fields[field]&&afCreditorKey(loan.fields[field])===afCreditorKey(e.value));});
 ...
 try{
  for(const item of files){
   ...
   try{
    let r;
    {
     const payload=await HostedAssessment.analyzeFile(item,preferences);r=HostedAssessment.adapt(payload);r.assessmentDate=payload.assessmentDay;
    }
...
$af('afExport').onclick=()=>{
 const missing=afMissing().length+afMissingGroups().length,pending=afPending().length,missingDocs=missingDocuments().length,lines=['КАРТОЧКА КЛИЕНТА','Дата оценки: '+$af('afDate').value,'Статус: '+(missing||pending||missingDocs||af.conflicts.length?'Черновик':'Ответы заполнены; требуется итоговая проверка специалиста'),`Осталось ответов: ${missing}; проверить извлечённых: ${pending}; недостающих документов: ${missingDocs}`,''];
 for(const card of $af('questionnaireStep').querySelectorAll('section.card')){lines.push(card.querySelector('.card-title')?.textContent.trim()||'');for(const e of card.querySelectorAll('input,select,textarea')){if(!afLogicalVisible(e)||e.closest('.af-source')||e.type==='file'||e.type==='password')continue;if(e.type==='checkbox'){if(e.checked)lines.push('• '+e.parentElement.textContent.trim());continue;}const src=af.sources.get(e.id);lines.push('• '+afLabel(e)+': '+(e.value||'НЕ ЗАПОЛНЕНО')+(src?' ['+(src.pending?'ПРОВЕРИТЬ':src.edited?'Изменено вручную':'Проверено')+'; '+(selectedFiles.find(x=>x.id===src.fileId)?.file.name||'документ')+', стр. '+src.page+']':''));}lines.push('');}
