...
function afRenderResults(){
 ...
 for(const item of selectedFiles){const r=af.results.get(item.id)||(item.storedDocumentId?{sourceOnly:true}:{pending:true});
  ...
  if(r.kind==='id'&&(r.reviewContext?.issuedAt||r.reviewContext?.expiresAt)){const dates=r.reviewContext,show=v=>v?.split('-').reverse().join('.')||'не прочитана';box.append(afEl('p','Дата выдачи: '+show(dates.issuedAt)+' · Действует до: '+show(dates.expiresAt),'hint'));}
