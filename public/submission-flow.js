window.SubmissionFlow={mount(anchor,status){
 ...
 let checked=null,attempt=null,latest=null,busy=false,generation=0;
 ...
 async function operate(action){if(busy)return;busy=true;save.disabled=resume.disabled=cancel.disabled=true;try{await action();}catch(error){status.textContent=error.message;}finally{busy=false;resume.disabled=cancel.disabled=false;save.disabled=false;await refresh();}}
 save.onclick=()=>operate(async()=>{
  if(!checked||currentDeal()!==checked.dealId||signature()!==checked.signature){
   ...
   if(!checked&&result?.answersComplete&&!result.evidence?.issues.length&&keysOnly){
    await window.CredentialUpload?.submit();await window.AssessmentCheck.run();
   }
  ...
  if(window.AssessmentDocumentUpload)await window.AssessmentDocumentUpload.submit();
 ...
 resume.onclick=()=>operate(async()=>{if(!latest)throw Error('Сохранение не найдено.');await advance(latest.dealId,latest);});
