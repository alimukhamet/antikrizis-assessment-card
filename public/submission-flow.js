window.SubmissionFlow={mount(anchor,status){
 ...
 // The browser never decides which CRM write or reconciliation comes next.
 // The server returns the saved contract or one actionable status for the same operation.
 async function finish(dealId,result,guard){
  if(currentDeal()!==dealId||(guard&&!guard()))throw Error('Клиент или ответы изменились во время сохранения. Уже отправленные данные не удалены. Проверьте текущую версию.');
  if(result.workflow?.status==='cancelled')attempt=null;
  if(!result.assessmentSaved||!result.historySaved||result.workflow?.status!=='ready'||!result.contract)throw Error(result.workflow?.message||'Сохранение не подтверждено. Нажмите «Скачать договор» ещё раз, чтобы проверить ту же операцию.');
  phase('Готовлю договор…','Готовлю сохранённый договор…');await download(dealId,result.contract);report('Договор готов. Если скачивание не началось, нажмите «Скачать готовый файл договора» ниже. Карточка, документы и история сохранены в сделке.');
 }
 ...
 save.onclick=()=>operate(async()=>{
  ...
  phase('Сохраняю…','Сохраняю договор и историю сделки. Повторно нажимать не нужно…');
  const row=await post(selected.dealId,{action:'complete',requestId:attempt.requestId,identityRevision:selected.identityRevision,payload:selected.payload,bindings:selected.bindings});
  await finish(selected.dealId,row,()=>currentDeal()===selected.dealId&&signature()===selected.signature);
 });
 resume.onclick=()=>operate(async()=>{const selected=latest,snapshot=signature();if(!selected)throw Error('Сохранение не найдено.');await confirmDestination();phase('Сохраняю…','Продолжаю сохранённую операцию…');const result=await post(selected.dealId,{action:'resume',requestId:selected.requestId});await finish(selected.dealId,result,()=>currentDeal()===selected.dealId&&signature()===snapshot);});
