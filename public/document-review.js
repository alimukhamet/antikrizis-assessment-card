...
window.DocumentReview={render(container,result,dealId,selection,onSaved){
 ...
 const supported=['Удостоверение личности','Ф6 об отсутствии имущества','Справка ЕНПФ','Справка по выплатам пенсии и пособий','Выписка зарплатного банка','Выписка Kaspi Gold','Доверенность','ГКБ — краткий отчёт','ГКБ — полный отчёт'];
 ...
 for(const selected of selection.filter(s=>s.person==='Клиент'&&supported.includes(s.type))){
  ...
  const gkb=selected.type.startsWith('ГКБ');
  if(['Удостоверение личности','Доверенность','Справка ЕНПФ'].includes(selected.type)||gkb)input('issuedAt','Дата выдачи','date',known.issuedAt||'');
  ...
  input('confirmed',gkb?'Проверил: я сверил все договоры, суммы долга и просрочку краткого и полного ГКБ по оригиналам.':selected.type==='Доверенность'?'Я просмотрел все страницы и проверил владельца, срок и полномочия поверенного.':'Я просмотрел все страницы и проверил владельца, содержание и срок документа.','checkbox');
