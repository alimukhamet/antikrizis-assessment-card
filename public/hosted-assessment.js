...
window.HostedAssessment=(()=>{
 ...
 async function load(){
  ...
  try{
   ...
   afStatus(fresh.client.iin?'Сделка открыта. Выберите документы этого клиента.':'Документы можно прочитать, но автозаполнение и подтверждения заблокированы до указания ИИН в сделке.',!fresh.client.iin);afRefresh();document.dispatchEvent(new Event('assessment-case-opened'));
  }catch(e){afStatus(e.message,true);}finally{el('hostLoadDeal').disabled=false;}
 }
 const base=()=>'/api/assessment/'+context.client.external.dealId;
 async function analyzeFile(item,preferences={}){const path=item.storedDocumentId?base()+'/documents/'+encodeURIComponent(item.storedDocumentId)+'/analyze':base()+'/documents';const options=item.storedDocumentId?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cacheOnly:preferences.cacheOnly===true})}:{method:'POST',headers:{'Content-Type':'application/octet-stream','X-Document-Name':encodeURIComponent(item.file.name)},body:item.file};let result;try{result=await json(path,options);}catch(error){if(error.code!=='CACHE_REPROCESS_REQUIRED'||!item.storedDocumentId)throw error;result=await json(path,{...options,body:JSON.stringify({cacheOnly:false})});}if(result.documentId)item.storedDocumentId=result.documentId;return result;}
 ...
 function adapt(payload){
  ...
  const kinds={gkb_full:'gkbFull',gkb_short:'gkbShort',identity:'id',property:'f6',kaspi:'kaspi',power_of_attorney:'power',benefits:'benefits',enpf:'enpf'};
  const types={gkb_full:'ГКБ — полный отчёт',gkb_short:'ГКБ — краткий отчёт',identity:'Удостоверение личности',property:'Ф6 об отсутствии имущества',kaspi:'Выписка Kaspi Gold',power_of_attorney:'Доверенность',benefits:'Справка по выплатам пенсии и пособий',enpf:'Справка ЕНПФ'};
  const notes=(payload.findings||[]).map(f=>errors[f]||({TOTAL_DEBT_REQUIRES_RECONCILIATION:'Общую задолженность нужно сверить: есть отдельные суммы просрочки или санкций.',DETAILED_EXTRACTION_PENDING:'Подробные поля этого типа документа пока заполняются вручную.',OCR_OR_PAGE_REVIEW_REQUIRED:'Есть страницы, требующие распознавания или ручной проверки.',DOCUMENT_TYPE_UNVERIFIED:'Тип документа не установлен по содержимому.',CONTRACT_LIST_INCOMPLETE_OR_OTHER_ROLES:'Проверьте полноту списка и роль клиента в обязательствах.'}[f]||'Есть сведения, требующие ручной проверки.'));
  ...
  return {engineVersion:3,creditEvidence,kind:kinds[data.kind]||'other',type:types[data.kind]||'Другой документ',identity:{iin:data.identity.iin,fio:data.identity.name},fields,loans,properties:[],gambling:data.facts.find(f=>f.key==='statement.gambling'),statement:data.bankStatement?{...data.bankStatement,period:data.bankStatement.from+' — '+data.bankStatement.to}:null,pages:read.totalPages,pageText:read.pages.map(p=>p.text),date:data.issuedAt,hash:read.pdfSha256,ocrPages:[],notes,blocked:!payload.eligibleForAutofill,server,sourcePreview:'/api/assessment/'+server.dealId+'/documents/'+server.documentId+'?view=pdf'};
 }
 ...
 return {mount,ready,adapt,review,error,analyzeFile,uploadPath:()=>base()+'/documents',getContext:()=>context};
})();
