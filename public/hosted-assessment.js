...
window.HostedAssessment=(()=>{
 ...
 function recognitionRecovery(){
  if(document.getElementById('recognitionUpdateNotice'))return;
  const notice=document.createElement('section');notice.id='recognitionUpdateNotice';notice.className='af-workspace';notice.setAttribute('role','alert');
  const message=document.createElement('p');message.textContent='Обновилось чтение документов. Ответы и файлы сохранены.';
  const button=document.createElement('button');button.type='button';button.className='btn btn-main';button.textContent='Обновить документы';
  button.onclick=async()=>{
   if(af.busy)return;button.disabled=true;
   try{await afAnalyze({cacheOnly:true,restoreOnly:true});message.textContent=[...af.results.values()].some(r=>r.error)?'Не все документы удалось прочитать. Повторите обновление.':'Документы обновлены. Проверьте ответы и продолжите.';}
   catch{message.textContent='Обновление не завершено. Ответы сохранены. Повторите действие.';}
   finally{button.disabled=false;}
  };
  notice.append(message,button);document.getElementById('checkStatus')?.after(notice);
 }
 ...
 async function reviewMany(entries){
  ...
  for(let offset=0;offset<prepared.length;offset+=100){
   ...
   for(const item of batch){
    ...
    if(!saved?.review?.id){if(saved?.error==='EXTRACTION_VERSION_CHANGED')recognitionRecovery();failure ||= error(saved?.error);continue;}
