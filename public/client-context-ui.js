window.ClientContextUI=(()=>{
 ...
 const pageTitle=profile?'Профиль клиента':mode==='handoff'?'Передать юристам':'Сформировать договор',homePath=profile?'/profile-backfill':mode==='handoff'?'/lawyer-handoff':'/assessment-review';
 ...
 for(const [key,text,path]of profile?[]:[['contract','Сформировать договор','/assessment-review'],['handoff','Передать юристам','/lawyer-handoff']]){const a=make('a',text);a.dataset.clientPath=path;a.href=path;a.target='_top';if(key===mode)a.setAttribute('aria-current','page');nav.append(a);}
 bar.append(home,nav);wrap.prepend(bar);bar.after(make('h1',pageTitle,'ux-page-title'));
