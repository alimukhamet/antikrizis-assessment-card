window.ServerDrafts=(()=>{
 ...
 async function restore(options={}){if(loading||!HostedAssessment.ready())return;clearTimeout(timer);if(saveTask)await saveTask;loading=true;$('loadDraft').disabled=true;try{if(!options.automatic&&!await confirmRestore())return;const path=base(),before=JSON.stringify(capture()),response=options.draft?{draft:options.draft}:await api(path+'/draft');if(path!==base()||!response.draft)return;if(JSON.stringify(capture())!==before)throw Error('Ответы изменились во время загрузки. Они сохранены на экране; повторите открытие черновика.');const draft=response.draft,p=draft.payload;if(draft.identityRevision!==HostedAssessment.getContext().identityRevision)throw Error(message('CASE_IDENTITY_CHANGED'));missingFiles=[...p.pendingFiles];
  ...
  showDraftStatus('Черновик загружен · версия '+revision+'. Файлы восстановлены: '+selectedFiles.length+'.'+(failed.length?' Не удалось получить файлов: '+failed.length+'.':'')+(p.pendingFiles.length?' Не загруженные ранее файлы нужно выбрать снова: '+p.pendingFiles.length+'.':''));
