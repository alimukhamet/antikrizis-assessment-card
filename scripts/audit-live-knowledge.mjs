...
try{
 ...
 const page=await request('/knowledge',{...options,headers});assert.equal(page.status,200);const markup=await page.text();assert.match(markup,/Практика судов/);assert.equal((markup.match(/type="search"/g)||[]).length,1);assert.match(markup,/aria-label="Регионы и районные суды"/);assert.match(markup,/aria-label="Порядок сортировки"/);assert.match(markup,/aria-label="По возрастанию процента"/);assert.match(markup,/aria-label="По убыванию процента"/);assert.doesNotMatch(markup,/<select\b/);assert.match(page.headers.get('cache-control'),/private, no-store/);report.unifiedCourtNavigation=true;report.sortDirectionControls=true;
