...
try{
 ...
 const page=await request('/knowledge',{...options,headers});assert.equal(page.status,200);assert.match(await page.text(),/База знаний/);assert.match(page.headers.get('cache-control'),/private, no-store/);
