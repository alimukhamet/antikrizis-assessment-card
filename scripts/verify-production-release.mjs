...
export function assertProductionRelease(env,dirty){
 if(env.GITHUB_ACTIONS!=='true'||env.GITHUB_REPOSITORY!=='alimukhamet/antikrizis-assessment-card'||!['Deploy assessment to Cloudflare','Assessment checks'].includes(env.GITHUB_WORKFLOW))throw Error('Production deploy blocked: commit changes and merge a pull request into main. The verified GitHub workflow publishes the combined application; local checkouts must not replace production.');
